...
import { safeExecute } from "./shared/utils";
...
function generateSignal(indicators: any, latestClose: number) {
  ...
  if (latestRSI < 50 && latestMACD.MACD > latestMACD.signal) {
  ...
  if (latestStochRSI !== null) {
    console.log('StochRSI:', latestStochRSI);
...
async function realBuy(price: number) {
  if (!BINANCE_API_KEY || !BINANCE_API_SECRET) {
    console.log('No API keys, simulating buy...');
    return;
  }
  try {
...
async function realSell(price: number) {
  if (SIMULATE_TRADES) {
    console.log('Simulated SELL at', price);
    return;
  }
  try {
    const order = await client.order({
...
async function openPosition(price: number) {
  position = 'LONG';
  entryPrice = price;
  balance -= price * parseFloat(QUANTITY);
  ...
  if (SIMULATE_TRADES) {
    ...
    console.log(`Simulated BUY`, simulatedOrder);
  } else {
    try {
      const order = await client.order({
        symbol: SYMBOL,
    ...
    console.log(`Real BUY executed at ${ price }`);
  }
...
app.listen(PORT, async () => {
  ...
  setInterval(() => safeExecute(analyzeMarket), parseInt(CHECK_INTERVAL, 10));
});
